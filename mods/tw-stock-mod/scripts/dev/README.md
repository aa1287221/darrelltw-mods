...
`chart-view.mjs`, `fit-rows.mjs`, `feed-errors.mjs`, `feed-budget.mjs`, `spawn-safety.mjs`, `crypto-feed.mjs`,
...
present, else `python3`) → `feed-errors.mjs` → `feed-budget.mjs` → `spawn-safety.mjs` → `crypto-feed.mjs` → `crypto-sort.mjs` →
...
regression. `feed-errors`, `feed-budget`, `spawn-safety`, `crypto-feed`, `crypto-sort` and `market-select`
