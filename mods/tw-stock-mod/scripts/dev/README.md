...
`chart-view.mjs` and `fit-rows.mjs` use it - they are the harnesses this exits non-zero on a real regression,
...
`tf-feed.mjs` → `tf-pnl.mjs` → `tabs.mjs` → `chart-view.mjs` → `fit-rows.mjs` → `pytest` (`scripts/tests`, via
